...
En poursuivant nos tests, on constate que l'ajout de la lettre "a" à la fin d'un mot n'affecte pas le chiffrement tant que la taille du mot ne dépasse pas le prochain multiple de cinq. On en concle que l'algorithme utilise la lettre "a" pour effectuer un remplissage (padding) sur le mot afin de le rendre d'une taille multiple de cinq.   
